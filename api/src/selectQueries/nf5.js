const nf5Queries = {
  getOrders: (
    limit,
    customerId
  ) => `SELECT oi5.order_id AS order_id, first_name AS customer_first_name, last_name AS customer_last_name,
    email AS customer_email, order_date, pm.method_name AS payment_method,
	  SUM(p.price * oi5.quantity) OVER (PARTITION BY o.id) AS payment_amount, payment_fee, 
    ...
    FROM nf5.order_items oi5
      JOIN nf5.orders o ON oi5.order_id = o.id
      ...
      JOIN nf5.product_supplier ps ON oi5.product_supplier_id = ps.id
      JOIN nf5.supplier_warehouse sw ON oi5.supplier_warehouse_id = sw.id
  ...
  getProductsStock: (
    limit,
    supplier_id
  ) => `SELECT ps5.product_name, c.category_name, s.supplier_name,
        ...
        ad.apartment AS warehouse_apartment, ps5.price
    FROM (
      SELECT 
        ps.product_id,
        ps.supplier_id,
    ...
    JOIN nf5.products ps5 ON ps5.id = psw.product_id
    JOIN nf5.categories c ON c.id = ps5.category_id
